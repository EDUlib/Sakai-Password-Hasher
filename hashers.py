...
#
# Added in order to support Sakai's password hashing algorithm
#
...
class SakaiPasswordHasher(BasePasswordHasher):
    """
    Big Test
    """
    algorithm = "sakai"
    ...
    def encode(self, password, salt, iterations=None):
        encrypted = gateway.encrypt(password)
        return "sakai$1$" + encrypted

    def verify(self, password, encoded):
        algorithm, iterations, salt, hash = encoded.split('$', 3)
        assert algorithm == self.algorithm
        return gateway.check(password, salt + '$' + hash)
